...
function build() {
  ...
  go build -mod=mod -buildmode=pie -ldflags "-s -extldflags=-Wl,-z,now  -X github.com/chaolihf/mind-cluster/component/npu-exporter/versions.BuildName=${OUTPUT_NAME} \
            -X github.com/chaolihf/mind-cluster/component/npu-exporter/versions.BuildVersion=${build_version}_linux-${arch}" \
    -o ${OUTPUT_NAME}
  ls ${OUTPUT_NAME}
  if [ $? -ne 0 ]; then
